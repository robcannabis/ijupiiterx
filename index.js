...
client.on('ready', async () => {
 ...
 const num = parseFloat((Math.random() * 0.2 + 0.1 + Number.EPSILON).toFixed(1));
  ...
  setInterval(() => {
    ...
    const ddds = ['https://discord.gg/ZNyP6qct']; // ลิ้ง button1

   const iooi = ['IG⭐']; // ชื่อ button 2
   const iiio = ['https://www.instagram.com/pxach9999/']; // ลิ้ง button 2
...
change[Math.floor(Math.random()*change.length)]; // รูปใหญ่
    ...
    const r = new Discord.RichPresence()
      ...
      .setType('STREAMING')
      .setURL('https://www.youtube.com/watch?v=16-wFAbvQiU')
